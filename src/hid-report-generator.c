...
enum KeyNum_t ScanCodeToKeyNum ( uint32_t activeLayers, uint32_t scanCode
                                 , uint32_t matrix ) {
        int8_t currentLayer;
        uint32_t layerMask = (1<<(NumLayers-1));
        for( currentLayer = NumLayers - 1
                     ; currentLayer >= 0
                     ; --currentLayer, layerMask >>= 1 )
                if ((activeLayers & layerMask)
                     && (layers[currentLayer][matrix][scanCode] != KB_Transparent))
...
*/
 inputReport_t * ScanCodesToReport (uint32_t leftFingers,  uint32_t leftThumbs
         ...
         uint32_t matrices[4] = {leftFingers, leftThumbs
                                 , rightFingers, rightThumbs};
         uint32_t matricesDelta[4] = {0, 0, 0, 0};
         inputReport_t *nextReport = &reportBuffer[(currentReport + 1)
                                                   % NumReports];
         for( byteCounter = 0; byteCounter < BytesPerReport; ++byteCounter )
         ...
         for( byteCounter = 0; byteCounter < 4; ++byteCounter)
                 matricesDelta[ byteCounter ] =
                         matrices[ byteCounter ] ^ oldMatrices[ byteCounter];
